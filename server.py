...
async def root():
    ...
    return JSONResponse({"status": "LUMINA API running", "version": "2.0.1"})
...
async def run_pipeline_logic(repo_url: str, emit_fn, user_token: Optional[str] = None):
    """
    Logic for running the pipeline (real or demo).
    Attempts real pipeline if all credentials are available, otherwise runs demo.
    """
    # Check if we have all required API keys for the real pipeline
    openai_key = os.environ.get("OPENAI_API_KEY", "")
    has_openai = bool(openai_key) and not openai_key.startswith("sk-ijkl")
    has_pinecone = bool(os.environ.get("PINECONE_API_KEY"))
    has_token = bool(user_token)

    if has_openai and has_pinecone and has_token:
        ...
        try:
            ...
            logger.warning(f"Real pipeline unavailable (missing deps): {e}")
            await emit_fn({"type": "log", "message": "Pipeline dependencies not available — running demo mode"})
        except Exception as e:
            logger.error(f"Real pipeline error: {e}")
            await emit_fn({"type": "log", "message": f"Pipeline encountered an issue — running demo mode"})
    else:
        reasons = []
        if not has_token:
            reasons.append("login with GitHub to use your token")
        if not has_openai:
            reasons.append("OpenAI API key not configured")
        if not has_pinecone:
            reasons.append("Pinecone API key not configured")
        msg = "Running demo mode — " + "; ".join(reasons) if reasons else "Running demo mode"
        await emit_fn({"type": "log", "message": msg})

    # Fall back to demo pipeline
        ...
        "report": {
            ...
            "vulnerabilities": [
                ...
                },
                {
                    ...
                    "type": "SQL Injection", "confirmed_type": "SQL Injection",
                    "severity": "HIGH",
                    "code": 'query = "SELECT * FROM users WHERE id = " + user_id',
                    "detail": "Unsanitized user input in SQL query",
                    "explanation": "Direct string concatenation in SQL query allows injection",
                    "cwe": "CWE-89", "verified": True,
                },
                {
                    "file": "app/utils.py", "line": 8,
                    "type": "Weak Cryptography", "confirmed_type": "Weak Cryptography",
                    "severity": "MEDIUM",
                    "code": 'hashlib.md5(password.encode()).hexdigest()',
                    "detail": "MD5 is cryptographically broken",
                    "explanation": "MD5 should not be used for password hashing",
                    "cwe": "CWE-327", "verified": True,
                },
            ],
            "fixes": [
                {
                    ...
                    "explanation": "Use environment variable instead of hardcoded value",
                ...
                },
                {
                    ...
                    "patched_line": 'query = "SELECT * FROM users WHERE id = %s"; cursor.execute(query, (user_id,))',
                ...
                },
                {
                    ...
                    "patched_line": "bcrypt.hashpw(password.encode(), bcrypt.gensalt())",
                    "explanation": "Use bcrypt for secure password hashing",
                    "diff_summary": "Replace MD5 with bcrypt",
                    "severity": "MEDIUM", "vuln_type": "Weak Cryptography",
                },
            ],
            "architecture": {
                "tech_stack": ["Python", "FastAPI", "SQLAlchemy", "Redis"],
                "components": {"api": "REST API", "auth": "JWT Auth", "db": "PostgreSQL"},
            },
        },
    })
